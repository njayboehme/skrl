...
# class AdaptiveDecoderNetwork(nn.Module):
#     def __init__(self, model_params, *args, **kwargs):
#         super().__init__(*args, **kwargs)
#         decoder_layer = AdaptiveDecoderLayer(d_model=model_params['d_model'], 
#                                                    nhead=model_params['nhead'],
#                                                    dim_feedforward=model_params['dim_feedforward'],
#                                                    batch_first=model_params['batch_first'],
#                                                    norm_first=model_params['norm_first'])
#         self.decoder = AdaptiveDecoder(decoder_layer, num_layers=model_params['num_layers'], )
...
#         self.norm3 = nn.LayerNorm(d_model, eps=layer_norm_eps, bias=bias, elementwise_affine=False, **factory_kwargs)
#         if scale not in [4, 6]:
#             raise ValueError(f"Scale must be 4 or 6. Got {scale}.")
#         self.scale = scale
#         # TODO: adaln should be 6 or 9 when using a decoder, but adaln probably not needed for decoder
