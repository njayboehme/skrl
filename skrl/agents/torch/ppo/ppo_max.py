...
from .ppo_cfg_max import PPO_CFG_MAX
...
def compute_gae(
    *,
    ...
    # min_reward = -50
    # max_reward = 30
    # squash rewards between -1 and 1
    # rewards = 2 * (rewards - min_reward) / (max_reward - min_reward) - 1
...
class PPO_Max(Agent):
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            device=device,
            cfg=PPO_CFG_MAX(**cfg) if isinstance(cfg, dict) else cfg,
        )
        ...
        if self.cfg.max_reward is not None:
            self.max_reward = self.cfg.max_reward
        
        self.entropy_loss_scale = None
        if self.cfg.entropy_loss_scale is not None:
    ...
    def update(self, *, timestep: int, timesteps: int) -> None:
        ...
        if self.entropy_loss_scale is not None and self.max_reward is not None:
            do_static = True
            if do_static:
                self.entropy_loss_scale *= 0.999
            else:
                # TODO: Do some update rule here using explained variance and max reward
                cur_rewards = self.memory.get_tensor_by_name("rewards").mean().item()
                max_rews = self.memory.get_tensor_by_name("rewards").max(dim=1)[0]
                max_rew = max_rews.max().item()
                if max_rew > self.max_reward:
                    self.max_reward = max_rew
                cur_rewards = max_rews[0].mean().item()
                reward_diff = self.max_reward - cur_rewards
                self.high_reward_gap = 10
                self.ent_decay_factor = 0.9
                self.ent_scale_factor = 1.0001
                self.ent_step = 0.0001
                self.ent_max = 0.01
                self.track_data("Test / Reward Diff", reward_diff)
                self.track_data("Test / Max Reward", self.max_reward)
                # High EV
                if explained_variance > 0.8:
                    # High reward diff (policy is bad and critic is good at predicting bad states)
                    if reward_diff > self.high_reward_gap:
                        # Increase the entropy significantly
                        self.entropy_loss_scale = min(self.ent_max, max(0, self.entropy_loss_scale + self.ent_step))
                        # self.entropy_loss_scale = min(self.ent_max, max(0, (self.entropy_loss_scale * self.ent_scale_factor) + self.ent_step))
                    # Low reward diff (policy is good and critic is good)
                    else:
                        self.entropy_loss_scale *= self.ent_decay_factor
                        if self.entropy_loss_scale < 1e-10:
                            self.entropy_loss_scale = 0.0
                # High reward difference and Low EV (policy is bad and critic is bad)
                elif reward_diff > self.high_reward_gap:
                    # Increase the entropy slightly
                    self.entropy_loss_scale = min(self.ent_max, max(0, self.entropy_loss_scale + self.ent_step))
        ...
        for epoch in range(self.cfg.learning_epochs):
            ...
            for (
                ...
                with torch.autocast(device_type=self._device_type, enabled=self.cfg.mixed_precision):
                    ...
                    if self.entropy_loss_scale is not None:
        ...
        self.track_data("Learning / Explained Variance", explained_variance)
        if self.entropy_loss_scale is not None:
            self.track_data(
            ...
            if self.max_reward is not None:
                self.track_data(
                "Learning / Entropy Coefficient", self.entropy_loss_scale
            )
